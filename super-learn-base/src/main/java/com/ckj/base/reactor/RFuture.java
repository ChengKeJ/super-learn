...
public interface RFuture<V> extends java.util.concurrent.Future<V>, CompletionStage<V> {
    ...
    /**
     * Return the result without blocking. If the future is not done yet this will
     * return {@code null}. As it is possible that a {@code null} value is used to
     * mark the future as successful you also need to check if the future is really
     * done with {@link #isDone()} and not relay on the returned {@code null} value.
     *
     * @return object
     */
    V getNow();
    ...
    /**
     * Waits for this future to be completed within the specified time limit without
